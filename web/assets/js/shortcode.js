...
(function($)
{
    $.fn.mTwitterIntergrator = function(options){
		...
		return this.each(function(){
			...
			jQuery.getJSON('http://api.twitter.com/1/statuses/user_timeline/'+opts.userName+'.json?count='+opts.noOfTweets+'&callback=?', function(tweets){
				//jQuery('.loading-tweet-53').fadeOut(1000,function(){jQuery("#twitter-update-list-53").append(twitterCallback2(tweets)).slideDown(1000);});
				
				$(".loading-text").fadeOut(1000,function(){
					$("ul#twitter_update_list").append(twitterCallback2(tweets)).slideDown(1000);
				});
			});		
